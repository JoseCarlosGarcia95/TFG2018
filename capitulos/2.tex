...
\begin{equation}
  ...
  X'(t) = f(t, X(t)), ~ X(0) = X_0
\end{equation}
Donde $f : [O, T] \times \mathcal{F}_\mathcal{H}(\IU) \rightarrow \mathcal{F}(\IR^n)$ se obtiene mediante el principio de extensión de Zadeh aplicada a una función continua $g : [0, T] \times U \rightarrow \IR^n$. Además, $f$  es continua por ser $g$ continua, además aplicando \hyperref[teorema:contfuzzy]{Teorema \ref*{teorema:contfuzzy}} podemos concluir:
\[
...
A continuación, se introducirá un resultado que permitirá resolver el problema \ref{eq:edo} se puede obtener una solución del problema \ref{def:edf}
